...
public class JwtUtil {
    ...
    public String generateToken(UserDetails userDetails) {
        ...
        if (user.isPresent()) {
            ...
            claims.put("roles", user.get().getRoles().stream().map(role -> "ROLE_" + clientCategory + "_" + role.getRole()).toList());
        }
        return doGenerateToken(claims, userDetails.getUsername());
    }

    private String doGenerateToken(Map<String, Object> claims, String subject) {
        return Jwts.builder().claims(claims).subject(subject).issuedAt(new Date(System.currentTimeMillis())).expiration(new Date(System.currentTimeMillis() + expiration * 1000)).signWith(getSigningKey()).compact();
    }
